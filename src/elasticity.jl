...
writevtk(
    Ω,
    "outputs/$(basename(@__FILE__))_results",
    cellfields = ["uh" => uh, "epsi" => ε(uh), "sigma" => σ ∘ ε(uh)],
);
...
writevtk(
    Ω,
    "outputs/$(basename(@__FILE__))_results_bimat",
    cellfields = ["uh" => uh, "epsi" => ε(uh), "sigma" => σ_bimat ∘ (ε(uh), tags)],
);
