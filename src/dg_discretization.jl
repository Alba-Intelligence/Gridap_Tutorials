...
∇u(x) = VectorValue(3 + x[2] * x[3], 2 * x[2] + x[1] * x[3], 6 * x[3]^2 + x[1] * x[2])
...
writevtk(Λ, "outputs/$(basename(@__FILE__))_strian");
...
a_Λ(u, v) =
    ∫(
        -jump(v * n_Λ) ⊙ mean(∇(u)) - mean(∇(v)) ⊙ jump(u * n_Λ) +
...
writevtk(Λ, "outputs/$(basename(@__FILE__))_jumps", cellfields = ["jump_u" => jump(uh)]);
